...
public class MASSISLauncher {
	...
	private static final String load_location = "load-simulation-from";
	...
	public static void main(String[] args) {
		...
		// Playback
		options.addOption(OptionBuilder
				.withLongOpt(load_location)
				.withDescription(
						"With " + simulation_mode + "=" + PLAYBACK
								+ ". The simulation file to load.").hasArg()
				.withArgName(SIMULATION_RESULTS_FILE).create());
		...
		try
		{
			...
			else if (SIMULATOR.equals(launch_optionValue))
			{
				...
				else if (simulationMode.equals(PLAYBACK))
				{
					...
					if (line.hasOption(load_location))
					{
						loadLocation = line.getOptionValue(load_location);

					}
					else
					{
						System.err.println(load_location + " is mandatory.");
						formatter.printHelp(helpHeader, options);
						return;
					}
					if (!line.hasOption(display))
					...
					simulate(buildingFilePath, resourceFolderPath,
							loadLocation, displayMode.equals(GUI), true, runFor);
				}
				else
				{
			...
			if (playback)
			{
				AbstractSimulation
						.runSimulation(RecordedSimulation.class,
								new String[] { "-building", buildingFilePath,
			...
			else
			{
				if (logFileLocation == null)
					AbstractSimulation.runSimulation(Simulation.class,
							new String[] { "-building", buildingFilePath,
									"-resources", resourceFolderPath, "-for",
									String.valueOf(runFor) });
				else
