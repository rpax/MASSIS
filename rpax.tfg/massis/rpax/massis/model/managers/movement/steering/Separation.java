...
public class Separation extends SteeringBehavior {
	...
	public Separation(Agent v, double d) {
		super(v);
	...
	private static KVector separateNew(Agent v,double STEPS_AHEAD) {
		KVector force = new KVector();
		KVector brakingforce = new KVector();
		int inminentCollisionCount = 0;
		List<KLine> agentCollisionLines = Steering.getCollisionLines(v, STEPS_AHEAD);
		...
		for (Agent other : v.getAgentsInRange(STEPS_AHEAD * agent_vel_magnitude))
		{
			...
			{
				List<KLine> otherCollisionLines = Steering.getCollisionLines(other,
						STEPS_AHEAD);
...
//
	...
	private static KVector separateOld(Agent v,double STEPS_AHEAD) {

		int minStep = Integer.MAX_VALUE;
		KPoint otherCenterAtIntersection = null;
		KPoint vCenterAtIntersection = null;
		Agent nearestOther=null;
		KVector force = new KVector();
		
		for (Agent other : v.getAgentsInRange(v.getMaxSpeed() * (STEPS_AHEAD + 1)))
		{
			if (v == other)
				continue;
...
//				continue;
		...
		if (otherCenterAtIntersection != null && vCenterAtIntersection != null)
		{
			...
			double numberOfStepsToCollision = v.getMaxSpeed() / KPoint.distance(v.getXY(), vCenterAtIntersection);
			force.mult(0.3 / (numberOfStepsToCollision*numberOfStepsToCollision));
