...
public class CollisionAvoidance extends SteeringBehavior {
	...
	public CollisionAvoidance(Agent v, double d) {
		super(v);
	...
	public KVector steer() {
		...
		for (Agent other : v.getAgentsInRange(STEPS_AHEAD * 2
				* agent_vel_magnitude))
		{
			...
			if (t > 0 && t < minT)
			{
				...
				repulsionVector = KVector.sub(futureV, futureO).normalize().div(t * 0.1);
	...
	public static KVector futureLocation(Agent a, double t) {
		return new KVector(a.getXY()).add(a.getVelocity().copy().mult(t));
	}

	private static double getTimeToCollision(Agent a, Agent b, double d1) {
