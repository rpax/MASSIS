...
import rpax.massis.model.managers.movement.Path;
...
import rpax.massis.util.pathfinding.straightedge.SEPathFinder;
...
public class Floor implements Indexable, MetadataConstants {
	/**
	...
	private static final int getNewUID() {
	 ...
	 * Rooms & doors
	 */
	private final List<SimDoor> doors = new ArrayList<SimDoor>();
	private final List<SimWindow> windows = new ArrayList<SimWindow>();
	private final ArrayList<RoomConnector> roomConnectors = new ArrayList<RoomConnector>();
	 ...
	 * QuadTree
	 */
	private final ArrayQuadTree<Agent> quadPilu;
	...
	private void initializeSimFurniture() {
		for (HomePieceOfFurniture f : this.furniture3D)
		{
			...
			Map<String, String> metadata = MASSISHomeMetadataManager
					.getHomeMetaData(building.getHome()).getMetadata(f);
			/*
			 * Special case: teleports
			 */
			if (metadata.containsKey(TELEPORT))
			{

				Teleport teleport = new Teleport(

				metadata, location, this.building.getMovementManager(),
						this.building.getAnimationManager(),
						this.building.getEnvironmentManager(),
						this.building.getResourcesFolder()
			 ...
			 */
			else if (metadata.containsKey(POINT_OF_INTEREST))
			{
				this.building.addNamedLocation(metadata.get(NAME),
			...
			else if (f instanceof HomeDoorOrWindow)
			{
				...
				if (f.getName() != null
						&& f.getName().toLowerCase().contains(WINDOW))
				{
			...
			else
			{
				 ...
				 */
				Agent person = Agent.createAgent(building.getHome(), f,
	...
	private void initializeWalls() {
		for (Wall w : this.walls3D)
		{
			...
			Map<String, String> metadata = MASSISHomeMetadataManager
					.getHomeMetaData(building.getHome()).getMetadata(w);
	...
	private void initializeRooms() {
		for (Room r : this.rooms3D)
		{
			...
			Map<String, String> metadata = MASSISHomeMetadataManager
					.getHomeMetaData(building.getHome()).getMetadata(r);
	...
	public SimRoom getRandomRoom() {
		...
		SimRoom room = this.rooms.get(ThreadLocalRandom.current().nextInt(
				this.rooms.size()));
		return room;
	}
	...
	public List<SimRoom> getRooms() {
		return rooms;
	}

	public Iterable<Agent> getPeople() {
	...
	public void addPerson(SimulationObject simObj) {
		if (simObj instanceof Agent)
		{
			this.quadPilu.insert((Agent) simObj);
	...
	public List<Teleport> getTeleportsConnectingFloor(final Floor other) {
		...
		{
			ArrayList<Teleport> teleportsConnecting = new ArrayList<Teleport>();
			for (Teleport teleport : this.teleports)
			{
				if (teleport.getType() == Teleport.START
						&& teleport.getDistanceToFloor(other) < Integer.MAX_VALUE)
				{
	...
	public KPoint getNearestPointOutsideOfObstacles(double x, double y) {
		return this.pathFinder.getNearestPointOutsideOfObstacles(new KPoint(x,
				y));
	}
	 ...
	 */
	public Iterable<Agent> getAgentsInRange(int xmin, int ymin, int xmax,
			int ymax) {
	 ...
	 * 
	 */
	private static class SearchRangeCallback implements
			ArrayQuadTreeCallback<Agent> {
		private final ArrayList<Agent> agents = new ArrayList<>();

		@Override
		public void query(Agent element) {
			agents.add(element);
		}
