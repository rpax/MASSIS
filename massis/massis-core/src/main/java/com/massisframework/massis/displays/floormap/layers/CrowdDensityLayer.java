...
public class CrowdDensityLayer extends DrawableLayer<DrawableFloor> {
	 ...
	 */
	private final HashMap<DrawableFloor, BufferedImage> densityImages = new HashMap<>();
	...
	public void draw(DrawableFloor f, Graphics2D g)
	{
		...
		final double scaleFactor = IMG_MAX_SIZE
				/ ((f.getYlength() > f.getYlength()) ? f.getXlength()
						: f.getYlength());
		/**
		 * drawing width
		 */
		...
		// for (LowLevelAgent a : f.getAgents()) <--- Agent??
		for (SimulationEntity e : f.getEntitiesFor(Location.class,
				MovementCapabilities.class))
		{
			MovementCapabilities mc = e.get(MovementCapabilities.class);
			Location a = e.get(Location.class);
			/*
		...
		g.drawImage(
				...
				(int) f.getMinX(),
				(int) f.getMinY(),
				(int) (f.getMaxX() - f.getMinX()),
				(int) (f.getMaxY() - f.getMinY()), null);
	}
