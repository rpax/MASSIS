...
public class FloorMapLayersUtils {
	...
	{
		double inverse_percent = 1.0 - percent;
		int redPart = (int) (((color1 >> 16) & 0xFF) * percent
				+ ((color2 >> 16) & 0xFF)
						* inverse_percent);
		int greenPart = (int) (((color1 >> 8) & 0xFF) * percent
				+ ((color2 >> 8) & 0xFF)
						* inverse_percent);
		int bluePart = (int) (((color1 >> 0) & 0xFF) * percent
				+ ((color2 >> 0) & 0xFF)
						* inverse_percent);
			...
			double x2, double y2, double d, double h)
	{
		...
		int[] xpoints = {
				(int) x2, (int) xm, (int) xn
		};
		int[] ypoints = {
				(int) y2, (int) ym, (int) yn
		};
	...
	public static void drawLine(Graphics2D g, CoordinateHolder p1,
			CoordinateHolder p2)
	{
	...
	public static void drawRect(Graphics2D g, double x, double y, double width,
			double height)
	{
		g.drawRect((int) (x), (int) (y), (int) (width), (int) (height));
	}

	public static void drawRectAtCenter(Graphics2D g, double xCenter,
			double yCenter, double width, double height)
	{
		drawRect(g, xCenter - width / 2, yCenter - height / 2, width, height);
	}

	public static void drawRectAtCenter(Graphics2D g, KPoint center,
	...
	public static Iterable<Shape> getShapes(Iterable<SimulationEntity> entities)
	{
		return StreamSupport
				.stream(entities.spliterator(), false)
				.map(FloorMapLayersUtils::getShape)::iterator;
	}
}
