...
public class Separation extends SteeringBehavior {
	...
	public Separation(SimulationEntity v, double d)
	{
		super(v);
	...
	private static KVector separateNew(SimulationEntity v, double STEPS_AHEAD)
	{
		...
		List<KLine> agentCollisionLines = Steering.getCollisionLines(v,
				STEPS_AHEAD);
		final double agent_vel_magnitude = v.getComponent(Velocity.class)
				.getValue().magnitude();
		KPoint xy = v.getComponent(Position2D.class).getXY().copy();
		double agentMinDistToIntersection = Double.MAX_VALUE;
		KPoint nearestIntersection = null;
		double collisionTimeDiff = v.getComponent(ShapeComponent.class)
				.getRadius()
				/ agent_vel_magnitude;
		KVector vCenterAtIntersection = null;
		KVector otherCenterAtIntersection = null;
		for (SimulationEntity other : getActiveAgentsInRange(v,
				STEPS_AHEAD * agent_vel_magnitude))
		{
			KPoint otherxy = other.getComponent(Position2D.class).getXY();
			double agent_other_dist = otherxy.distance(xy);
			boolean isCollisionInminent = agent_other_dist < v
					.getComponent(ShapeComponent.class).getRadius()
					+ other.getComponent(ShapeComponent.class).getRadius();
			if (isCollisionInminent)
			{
				brakingforce.add(
						KVector.sub(xy, otherxy)
								// KVector.normal(new KVector(),
								// other.getVelocity()).mult(-1)
								.normalize()
								.mult(1 / (agent_other_dist
										* agent_other_dist)));
			...
			if (inminentCollisionCount == 0)
			{
				List<KLine> otherCollisionLines = Steering.getCollisionLines(
						other,
						STEPS_AHEAD);
				final double other_vel_magnitude = other.getComponent(Velocity.class).getValue()
						.magnitude();
				for (KLine agentLine : agentCollisionLines)
				{
					for (KLine otherLine : otherCollisionLines)
					{
						...
						if (intersection != null && 
								v.getRoom().getPolygon() .contains(intersection)
						   )
						{
		...
		{
			force = brakingforce/*
								 * .normalize().sub(KVector.normalize(v.
								 * getVelocity()))
								 */.normalize();
		...
		} else if (nearestIntersection != null)
		{
			...
			force = KVector
					.sub(vCenterAtIntersection, otherCenterAtIntersection)
					.normalize();// .mult(10000);
	...
	private static KVector separateOld(LowLevelAgent v, double STEPS_AHEAD)
	{
		...
		if (otherCenterAtIntersection != null && vCenterAtIntersection != null)
		{
			...
			force.mult(
					0.3 / (numberOfStepsToCollision
							* numberOfStepsToCollision));
