...
public class SimLocation extends LocationImpl implements Restorable {
	...
	/**
	 * @param polygon
	 *            the polygon representing the element
	 * @param floor
	 *            the floor
	 */
	 ...
	 *
	 * @param hpof
	 *            the SH3D representation
	 * @param floor
	 *            the floor of the ekement
	 */
	...
	public void translateTo(double other_x, double other_y, Floor other_floor)
	{
		final double oldX = this.getX();
		final double oldY = this.getY();
		final double toX = other_x;
		final double toY = other_y;
		...
		if (this.floor != other_floor)
		{
			Logger.getLogger(SimLocation.class.getName()).log(
					...
					{
							this.floor.getName(), other_floor.getName()
					});
			this.floor.remove(attached);
		}
		other_floor.addPerson(attached);
		this.floor = other_floor;
