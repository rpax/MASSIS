...
public class Steering {
			...
			List<ContainmentPolygon> containmentPolygons)
	{
		...
		// shortcuts
		if (person.getRoom().getBoundaryPointClosestTo(person.getXY())
				.distance(person.getXY()) > person
						.getMaxSpeed())
		{
			return new KVector(0, 0);
		...
		KVector[] forward = new KVector[thetas.length],
				ray = new KVector[thetas.length],
				projection = new KVector[thetas.length],
				acc = new KVector[thetas.length],
				lateral = new KVector[thetas.length];
		...
		for (int n = 0; n < thetas.length; n++)
		{
			...
			p.add(new KVector(ray[n].x - lateral[n].x,
					ray[n].y - lateral[n].y));
			p.add(new KVector(ray[n].x + lateral[n].x,
					ray[n].y + lateral[n].y));
	 ...
	 * <p>
	 * Much credit due to:
	 * <a href="http://www.openprocessing.org/user/8371">Jacob Haip's code</a>
	 * </p>
	 * <p>
	...
	private static KVector overlap(ContainmentPolygon ob1,
			ContainmentPolygon ob2)
	{ // ob1 is one that is projected
		// (moves)
