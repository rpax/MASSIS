...
import rpax.massis.pathfinding.straightedge.SEPathFinder;
...
public class Floor implements Indexable {
     ...
     *
     * @param level3D the level (1,2,3)
     * @param rooms3D the rooms in that level
     * @param walls3D the walls in that level
     * @param furniture3D the furniture in that level
     */
            ...
            ArrayList<HomePieceOfFurniture> furniture3D, Building building)
    {
        ...
        System.err.println(
                "======================================================");
        System.err.println(
                "Creating floor from level [" + this.level3D.getName() + "]");
    ...
    private void initializeWalls()
    {
        for (Wall w : this.walls3D)
        {
            ...
            Map<String, String> metadata = this.building.getMetadata(
                    w);
     ...
     *
     * @param simObj the agent to be removed
     */
    ...
    private HighLevelController createHLController(
            LowLevelAgent agent,
            ...
            String resourcesFolder)
    {
        ...
        final String className = metadata.get(
                SimObjectProperty.CLASSNAME.toString());
        ...
        if (className != null)
        {
            try
            {
                ...
                hlc = HighLevelController.newInstance(
                        agentClass, agent, metadata, absResFolder);


            } catch (ClassNotFoundException ex)
            {
                Logger.getLogger(Floor.class.getName()).log(
    ...
    private static class SearchRangeCallback implements
            ArrayQuadTreeCallback<DefaultAgent> {
