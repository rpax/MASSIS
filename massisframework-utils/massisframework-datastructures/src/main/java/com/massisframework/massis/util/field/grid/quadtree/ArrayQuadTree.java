...
public class ArrayQuadTree<E extends XYHolder> extends Field2D<E> {
	...
	public void insert(E e) {
		...
		final int x = (int) (e.getX_2D() * this.inv_cellSizeX - this.inv_cellSizeX_minX);
		final int y = (int) (e.getY_2D() * this.inv_cellSizeY - this.inv_cellSizeY_minY);
	...
	private void searchInRange(int level, int index, double x0, double y0,
		...
		if (level == this.noLevels-1)
		{
			...
			for (final E elem : this.childNodes[x][y])
			{
				...
				if (contains(rx0, ry0, rx1, ry1, elem.getX_2D(), elem.getY_2D()))
				{
					callback.query(elem);
				}
			}
